...
public class PolygonImage : MaskableGraphic, ISerializationCallbackReceiver, ICanvasRaycastFilter
{
    ...
    void Update()
    {
        if (edgeWeights != null && temp.Count >= edgeWeights.Weights.Count)
        {
            for (int i = 0; i < edgeWeights.Weights.Count; i++)
            {
                //Debug.Log("--->" + temp[i]);
                edgeWeights.m_Weights[i] = Mathf.Lerp(edgeWeights.m_Weights[i], temp[i], 0.2f);
                //DOTween.To(() => edgeWeights.m_Weights[i], (x) => edgeWeights.m_Weights[i] = x, Random.Range(0f, 1f), 1f); 
            }
        }
    ...
    public void UpdatePolygon()
    {
        ...
        for (int i = 0; i < edgeWeights.Weights.Count; i++)
        {
            ...
            //Debug.Log("---<M:" + temp[i]);
            //edgeWeights.m_Weights[i] = Random.Range(0f, 1f);
            //DOTween.To(() => edgeWeights.m_Weights[i], (x) => edgeWeights.m_Weights[i] = x, Random.Range(0f, 1f), 1f); 
        }
        //SetVerticesDirty();
    }
